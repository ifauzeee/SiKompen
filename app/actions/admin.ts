...
export async function updateStudentHours(
  studentId: number,
  newHours: number,
  reason: string,
) {
  ...
  try {
    ...
    if (!response.ok)
      return { error: data.error || "Gagal mengupdate jam hutang." };
...
export async function updateSystemSetting(key: string, value: string) {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok)
      return { error: data.error || "Gagal menyimpan pengaturan." };
...
export async function importStudents(
  data: { nim: string; name: string; prodi: string; kelas: string }[],
) {
  const session = await getSession();
  if (!session || session.role !== "ADMIN") {
    return { error: "Unauthorized" };
  ...
  try {
    ...
    if (!response.ok)
      return { error: resData.error || "Gagal mengimport data." };
