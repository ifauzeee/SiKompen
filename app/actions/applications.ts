...
export async function getApplicationsByStatus(
  status: "PENDING" | "ACCEPTED" | "VERIFYING" = "PENDING",
  supervisorId?: number,
) {
  ...
  try {
    ...
    if (supervisorId)
      url.searchParams.append("supervisorId", supervisorId.toString());
...
export async function updateApplicationStatus(
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok)
      return { error: data.error || "Gagal memproses validasi." };
...
export async function applyForJob(jobId: number) {
  ...
  try {
    ...
    if (!response.ok)
      return {
        error: data.error || "Terjadi kesalahan saat mengirim lamaran.",
      };
