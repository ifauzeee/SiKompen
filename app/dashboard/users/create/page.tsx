"use client";

import { useFormStatus } from "react-dom";
import { createUser } from "@/app/actions/users";
import {
  User,
  Shield,
  GraduationCap,
  Lock,
  AlertCircle,
  Building,
  BookOpen,
  UserCircle,
  Wallet,
} from "lucide-react";
import Link from "next/link";
import { useState } from "react";
import { useRouter } from "next/navigation";

function SubmitButton() {
  const { pending } = useFormStatus();
  return (
    <button
      disabled={pending}
      className="ml-auto flex items-center justify-center gap-3 rounded-xl bg-gray-900 px-8 py-4 text-lg font-bold text-white transition-all hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
    >
      {pending ? (
        <>
          <div className="h-5 w-5 animate-spin rounded-full border-2 border-white/30 border-t-white" />
          Menyimpan...
        </>
      ) : (
        "Simpan User Baru"
      )}
    </button>
  );
}

export default function CreateUserPage() {
  const router = useRouter();
  const [error, setError] = useState("");
  const [role, setRole] = useState("MAHASISWA");

  async function clientAction(formData: FormData) {
    setError("");
    const result = await createUser(formData);

    if (result?.error) {
      setError(result.error);
    } else {
      router.push("/dashboard/users");
      router.refresh();
    }
  }

  return (
    <div className="min-h-screen bg-gray-50/50">
      <div className="mx-auto max-w-4xl px-6 py-12">
        {error && (
          <div className="mb-6 flex items-start gap-3 rounded-xl border border-red-100 bg-red-50 p-4 text-red-600">
            <AlertCircle className="mt-0.5 shrink-0" size={20} />
            <span className="font-medium">{error}</span>
          </div>
        )}

        <form action={clientAction} className="space-y-8">
          <div className="space-y-2">
            <label className="ml-1 text-sm font-bold tracking-wider text-gray-500 uppercase">
              Peran Pengguna
            </label>
            <div className="grid grid-cols-3 gap-4">
              {[
                { id: "MAHASISWA", icon: GraduationCap, label: "Mahasiswa" },
                { id: "PENGAWAS", icon: User, label: "Pengawas" },
                { id: "KEUANGAN", icon: Wallet, label: "Keuangan" },
                { id: "ADMIN", icon: Shield, label: "Admin" },
              ].map((r) => (
                <label key={r.id} className="group relative cursor-pointer">
                  <input
                    type="radio"
                    name="role"
                    value={r.id}
                    className="peer sr-only"
                    checked={role === r.id}
                    onChange={() => setRole(r.id)}
                  />
                  <div className="flex flex-col items-center gap-3 rounded-2xl border-2 border-gray-100 bg-white p-6 text-center transition-all group-hover:border-gray-300 peer-checked:border-gray-900 peer-checked:bg-gray-900 peer-checked:text-white">
                    <r.icon size={24} />
                    <span className="text-sm font-bold">{r.label}</span>
                  </div>
                </label>
              ))}
            </div>
          </div>

          <div className="space-y-2 pt-4">
            <label className="ml-1 text-sm font-bold tracking-wider text-gray-500 uppercase">
              Nama Lengkap
            </label>
            <input
              name="name"
              placeholder="Nama Lengkap User"
              required
              className="w-full border-b-2 border-gray-200 bg-transparent px-0 py-4 text-2xl font-bold text-gray-900 transition-all outline-none placeholder:text-gray-300 focus:border-gray-900"
            />
          </div>

          <div className="grid grid-cols-1 gap-8 pt-2 md:grid-cols-2">
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-sm font-bold tracking-wider text-gray-500 uppercase">
                <UserCircle size={16} />
                Username
              </label>
              <input
                type="text"
                name="username"
                placeholder="Username atau NIM"
                required
                className="w-full rounded-2xl border border-gray-200 bg-white px-6 py-4 font-bold text-gray-900 transition-all outline-none focus:border-[#008C9D] focus:ring-4 focus:ring-[#008C9D]/10"
              />
            </div>

            <div className="space-y-2">
              <label className="flex items-center gap-2 text-sm font-bold tracking-wider text-gray-500 uppercase">
                <Lock size={16} />
                Password Default
              </label>
              <input
                type="text"
                name="password"
                placeholder="******"
                required
                className="w-full rounded-2xl border border-gray-200 bg-white px-6 py-4 font-bold text-gray-900 transition-all outline-none focus:border-[#008C9D] focus:ring-4 focus:ring-[#008C9D]/10"
              />
            </div>
          </div>

          {role === "MAHASISWA" && (
            <div className="animate-in fade-in slide-in-from-top-4 space-y-8 pt-4">
              <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-bold tracking-wider text-gray-500 uppercase">
                    <GraduationCap size={16} />
                    NIM
                  </label>
                  <input
                    name="nim"
                    placeholder="Nomor Induk Mahasiswa"
                    className="w-full rounded-2xl border border-gray-200 bg-white px-6 py-4 font-bold text-gray-900 transition-all outline-none focus:border-[#008C9D] focus:ring-4 focus:ring-[#008C9D]/10"
                  />
                </div>
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-bold tracking-wider text-gray-500 text-red-500 uppercase">
                    Total Hutang (Jam)
                  </label>
                  <input
                    type="number"
                    name="totalHours"
                    defaultValue={0}
                    className="w-full rounded-2xl border border-red-100 bg-red-50 px-6 py-4 font-bold text-red-600 transition-all outline-none focus:border-red-500 focus:ring-4 focus:ring-red-500/10"
                  />
                </div>
              </div>
              <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-bold tracking-wider text-gray-500 uppercase">
                    <Building size={16} />
                    Program Studi
                  </label>
                  <select
                    name="prodi"
                    className="w-full rounded-2xl border border-gray-200 bg-white px-6 py-4 font-bold text-gray-900 transition-all outline-none focus:border-[#008C9D] focus:ring-4 focus:ring-[#008C9D]/10"
                  >
                    <option value="">Pilih Program Studi</option>
                    <option value="Teknik Informatika">
                      Teknik Informatika
                    </option>
                    <option value="Teknik Multimedia dan Jaringan">
                      Teknik Multimedia dan Jaringan
                    </option>
                    <option value="Teknik Multimedia Digital">
                      Teknik Multimedia Digital
                    </option>
                  </select>
                </div>
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-bold tracking-wider text-gray-500 uppercase">
                    <BookOpen size={16} />
                    Kelas
                  </label>
                  <input
                    name="kelas"
                    placeholder="Contoh: TI 1A"
                    className="w-full rounded-2xl border border-gray-200 bg-white px-6 py-4 font-bold text-gray-900 transition-all outline-none focus:border-[#008C9D] focus:ring-4 focus:ring-[#008C9D]/10"
                  />
                </div>
              </div>
            </div>
          )}

          <div className="flex items-center justify-between border-t border-gray-200 pt-8">
            <Link
              href="/dashboard/users"
              className="font-bold text-gray-500 hover:text-gray-900"
            >
              Batal
            </Link>
            <SubmitButton />
          </div>
        </form>
      </div>
    </div>
  );
}
