...
export default function ApplicationList({
  ...
  if (optimisticApps.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-4xl border-2 border-dashed border-gray-100 bg-gray-50/50 py-12 text-center md:rounded-[2.5rem]">
        <div className="mb-4 flex h-16 w-16 rotate-3 transform items-center justify-center rounded-4xl bg-white shadow-xl shadow-gray-100 transition-transform duration-300 hover:rotate-6">
          <FileCheck size={32} className="text-[#008C9D]" />
        </div>
        <h3 className="mb-1 text-lg font-black text-gray-900">
          {title || "Semua Beres!"}
        </h3>
        <p className="mx-auto max-w-xs px-4 text-sm text-gray-500">
          {variant === "PENDING"
    ...
    <div className="space-y-4">
      {optimisticApps.map((app) => {
        ...
        return (
          <div
            key={app.id}
            className="group relative flex flex-col overflow-hidden rounded-3xl border border-gray-100 bg-white transition-all duration-300 hover:border-[#008C9D]/30 hover:shadow-xl hover:shadow-[#008C9D]/5"
          >
            <div
              className={`absolute top-0 bottom-0 left-0 w-1 ${
                variant === "VERIFYING" ? "bg-purple-500" : "bg-[#008C9D]"
              } rounded-r-full`}
            />
            ...
            <div className="flex flex-col justify-between gap-4 p-4 md:gap-6 md:p-5 lg:flex-row lg:items-center">
              <div className="flex-1 pl-2 lg:pl-0">
                <div className="mb-3 flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    <h4 className="line-clamp-1 font-bold wrap-break-word text-gray-900 transition-colors group-hover:text-[#008C9D]">
                      {app.user.name || "Mahasiswa"}
                    </h4>
                    <div className="mt-0.5 flex flex-wrap items-center gap-2">
                      <span className="rounded-lg border border-gray-200 bg-gray-100 px-2 py-0.5 text-[10px] font-bold whitespace-nowrap text-gray-500">
                        {app.user.nim}
                      </span>
                      <span className="rounded-lg border border-red-100 bg-red-50 px-2 py-0.5 text-[10px] font-bold whitespace-nowrap text-[#CE2029]">
                ...
                <div className="pl-0 md:pl-14">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-gray-400">
                    <span className="flex items-center gap-1.5 rounded-md bg-gray-50 px-2 py-1 whitespace-nowrap">
                    ...
                    {app.proofImage1 && (
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-md bg-purple-50 px-2 py-1 whitespace-nowrap text-purple-700 transition-colors hover:bg-purple-100"
                      >
                        <ImageIcon size={12} />
                        Lihat Bukti
                      </button>
              ...
              <div className="mt-2 flex w-full items-center gap-3 pl-0 lg:mt-0 lg:w-auto lg:border-l lg:border-gray-200 lg:pl-6">
                {variant === "PENDING" && (
                  <>
                    <button
                ...
                {variant === "ACCEPTED" && (
                  <button
                    disabled
                    className="flex h-12 flex-1 cursor-not-allowed items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-gray-100 px-6 font-bold whitespace-nowrap text-gray-400 lg:flex-none"
            ...
            {isExpanded && app.proofImage1 && (
              <div className="animate-in slide-in-from-top-2 border-t border-gray-100 px-5 pt-0 pb-5 duration-300">
                <div className="space-y-4 rounded-2xl bg-gray-50 p-4">
                  <h5 className="flex items-center gap-2 font-bold text-gray-700">
                    <Eye size={16} />
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <a
                      ...
                      className="group/link flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 transition-all hover:border-[#008C9D] hover:shadow-lg"
                    >
                      ...
                      <ExternalLink
                        size={16}
                        className="text-gray-300 group-hover/link:text-[#008C9D]"
                      />
                    </a>
